...
function create_ticket_auto(PDO $pdo, array $payload){
  // Available columns in your tickets table
  $wantCols = ['ticket_number','subject','requester_email','description','status','priority','created_by','created_at'];
  $cols = [];
  ...
  foreach ($cols as $c) {
    ...
    // Status defaults to 'Open' unless provided
    if ($c==='status') {
      $columns_list[] = $c;
    ...
    // Priority: pick safe value (DB default preferred)
    if ($c==='priority') {
      $columns_list[] = $c;
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if (isset($_POST['sched_create'])) {
    ...
    if ($nxt) { $pdo->prepare("UPDATE scheduled_tasks SET next_run_at=? WHERE id=?")->execute([$nxt->format('Y-m-d H:i:s'), $task_id]); }
    $pdo->commit();
  ...
  if (isset($_POST['run_now'])) {
    ...
    try {
      ...
      if ($task && (int)$task['active']===1){
        $pdo->beginTransaction();
        $pdo->prepare("INSERT INTO scheduled_task_runs (task_id, run_date) VALUES (?, CURDATE())")->execute([$task_id]);
        ...
        $ass = $pdo->prepare("SELECT user_id FROM scheduled_task_assignees WHERE task_id=?"); $ass->execute([$task_id]); $assignees=$ass->fetchAll(PDO::FETCH_COLUMN);
        ...
        if ($next) $pdo->prepare("UPDATE scheduled_tasks SET next_run_at=? WHERE id=?")->execute([$next->format('Y-m-d H:i:s'), $task_id]);
        $pdo->commit();
      }
...
// Legacy simple tasks
$tasks = $pdo->query("
  SELECT t.*, CONCAT_WS(' ',u.first_name,u.last_name) AS assignee_user
...
<div class="card">
  ...
  <div class="card-b" style="overflow:auto">
    <?php if(isset($_GET['msg'])): ?>
      <div class="badge badge-success" style="display:block;margin-bottom:1rem;">Action: <?= e($_GET['msg']) ?></div>
    <?php endif; ?>
