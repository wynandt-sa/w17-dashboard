...
/* SLA columns */
if (!col_exists($pdo,'tickets','sla_started_at'))        $pdo->exec("ALTER TABLE tickets ADD COLUMN sla_started_at DATETIME NULL");
if (!col_exists($pdo,'tickets','sla_paused_started_at')) $pdo->exec("ALTER TABLE tickets ADD COLUMN sla_paused_started_at DATETIME NULL");
if (!col_exists($pdo,'tickets','sla_paused_seconds'))    $pdo->exec("ALTER TABLE tickets ADD COLUMN sla_paused_seconds INT NOT NULL DEFAULT 0");
if (!col_exists($pdo,'tickets','last_sla_level'))        $pdo->exec("ALTER TABLE tickets ADD COLUMN last_sla_level TINYINT NOT NULL DEFAULT 0");
if (!col_exists($pdo,'tickets','last_sla_escalated_at')) $pdo->exec("ALTER TABLE tickets ADD COLUMN last_sla_escalated_at DATETIME NULL");
...
/* ----------------- Housekeeping function (can be reused by cron/runner) ----------------- */
function run_ticket_housekeeping(PDO $pdo): void {
  // 1) Auto-reopen elapsed Pending
  $due = $pdo->query("
  ...
  if ($open) {
    foreach ($open as $t) {
      $elapsed = sla_elapsed_business_seconds($pdo, $t);
...
/* ---------------- Run housekeeping now (and you can reuse this function from cron) ---------------- */
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  if (isset($_POST['save_default'])) {
    ...
    $filters = [
      ...
      'agent_id'    => (int)($_POST['f_agent_id'] ?? 0) ?: null,
  ...
  if (isset($_POST['update'])) {
    ...
    $old = $pdo->prepare("SELECT agent_id, status, priority, queue, related_to_id, pending_until, sla_started_at, sla_paused_started_at, sla_paused_seconds FROM tickets WHERE id=?");
    $old->execute([$id]);
    ...
    // If Pending -> Open, notify assignee
    if (($before['status'] ?? '') === 'Pending' && $status === 'Open') {
      $tnSt = $pdo->prepare("SELECT ticket_number,subject,queue,priority,agent_id FROM tickets WHERE id=?");
      $tnSt->execute([$id]);
      if ($meta = $tnSt->fetch(PDO::FETCH_ASSOC)) {
        if (!empty($meta['agent_id'])) {
          $getEmail = $pdo->prepare("SELECT email FROM users WHERE id=?");
          $getEmail->execute([(int)$meta['agent_id']]);
          if ($email = $getEmail->fetchColumn()) {
            $link = (defined('APP_BASE_URL')? rtrim(APP_BASE_URL,'/') : '').'/tickets.php#t'.$id;
            $msg  = "Ticket **{$meta['ticket_number']}** — *".($meta['subject'] ?? 'Ticket')."* has **re-opened** (was Pending).\n".
                    "Queue: `".($meta['queue'] ?? '-')."`  |  Priority: `".($meta['priority'] ?? '-')."`\n".
                    ($link ? "[Open in Ticketing]($link)" : "");
            @zulip_send_pm($email, $msg);
          }
        }
...
$hasExplicit = isset($_GET['no']) || isset($_GET['status']) || isset($_GET['agent_id']) || isset($_GET['queue']) || isset($_GET['location_id']);
$filters = [
  'no'          => null,
  'status'      => null,
  'agent_id'    => null,
...
if ($hasExplicit) {
  ...
  $filters['agent_id']    = (int)($_GET['agent_id'] ?? 0) ?: null;
...
/* ---- Quick views (mine / team) ---- */
$view = $_GET['view'] ?? null;          // 'mine' | 'team' | null
$teamAgentIds = [];
if ($view === 'team') {
  $teamAgentIds = [$me['id']];
  $mgr = null;
...
if ($filters['agent_id'])    { $where[] = "t.agent_id=?";          $params[] = (int)$filters['agent_id']; }
        ...
        <?php foreach($tickets as $t): $tid=(int)$t['id']; ?>
          <tr id="t<?= $tid ?>" class="row-openable" data-id="<?= $tid ?>">
            ...
            <td style="display:none">
              ...
              <?php if($is_admin): ?>
              <form method="post" class="inline" onsubmit="return confirm('Delete this ticket?')" style="display:none">
      ...
      <form method="post" enctype="multipart/form-data" class="grid grid-2">
        ...
        <div><label class="label">To (Agent)</label>
          <select name="agent_id" class="input"><option value="">— Unassigned —</option>
            <?php foreach($users as $uid=>$nm): ?><option value="<?= (int)$uid ?>"><?= e($nm) ?></option><?php endforeach; ?>
          </select>
        </div>
        <div><label class="label">Queue</label>
          <select name="queue" class="input" required>
            <?php foreach(($queueOptions ?: []) as $q): ?><option><?= e($q) ?></option><?php endforeach; ?>
          </select>
        </div>
        ...
        <div><label class="label">Related To</label>
          <select name="related_to_id" class="input"><option value="">—</option>
            <?php foreach($relatedKV as $rid=>$path): ?><option value="<?= (int)$rid ?>"><?= e($path) ?></option><?php endforeach; ?>
          </select>
        </div>
